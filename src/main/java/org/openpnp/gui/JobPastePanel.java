...
public class JobPastePanel extends JPanel {
    ...
    public JobPastePanel(JobPanel jobPanel) {
        ...
        captureAndPositionActionGroup =
                new ActionGroup(moveCameraToPadLocation, moveToolToPadLocation, moveCameraToBoardLocationNextAction, dispenseAtPadLocation, dispenseAtPadLocationNextAction);
        ...
        toolBar.add(btnPositionCameraBoardLocationNext);
        ...
        JButton btnDispensePadLocation = new JButton(dispenseAtPadLocation);
        btnDispensePadLocation.setHideActionText(true);
        toolBar.add(btnDispensePadLocation);        

        JButton btnDispensePadLocationNextAction = new JButton(dispenseAtPadLocationNextAction);
        btnDispensePadLocationNextAction.setHideActionText(true);
        toolBar.add(btnDispensePadLocationNextAction);        
    ...
    public final Action dispenseAtPadLocation = new AbstractAction() {
        {
            putValue(SMALL_ICON, Icons.dispense);
            putValue(NAME, "Dispense at pad");
            putValue(SHORT_DESCRIPTION, "Dispense paste at pad");
        }

        @Override
        public void actionPerformed(ActionEvent arg0) {

            Actuator dispenseActuator = MainFrame.get().getMachineControls().getSelectedTool().getHead().getActuatorByName("Dispenser");              

            UiUtils.submitUiMachineTask(() -> {
                if(getSelection().getType().equals(Type.Paste))
                {
                    Location location = Utils2D.calculateBoardPlacementLocation(boardLocation,
                            getSelection().getLocation());              
                
                    MovableUtils.moveToLocationAtSafeZ(dispenseActuator, location);

                    dispenseActuator.actuate(true);

                    dispenseActuator.actuate(false);
                }
    ...
    public final Action moveCameraToBoardLocationNextAction =
        new AbstractAction() { //$NON-NLS-1$
        {
            ...
            putValue(NAME, "JobPanel.Action.Job.Camera.PositionAtNextBoardLocation"); //$NON-NLS-1$
            putValue(SHORT_DESCRIPTION, "JobPanel.Action.Job.Camera.PositionAtNextBoardLocation.Description"); //$NON-NLS-1$
        }
